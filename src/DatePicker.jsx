...
export function DatePicker({value, onChange}) {
  ...
  return (
    <div className="date-picker-container">
      <button className="date-picker-button" onClick={() => setIsOpen(o =>!o)}>{ format(value , "MMM do, yyyy") }</button>
    ...
    <div className="date-picker">
      ...
      <div className="date-picker-grid-dates date-picker-grid">
        ...
        {visibleDates.map(date => (
          <button className={`date ${!isSameMonth(date, visibleMonth) && "date-picker-other-month-date" } ${isToday(date) && "today" } ${isSameDay(date, value) && "selected" }`}
             key={date.toDateString()}>{date.getDate()}</button>
        ))}
      </div>
    </div>
  )
